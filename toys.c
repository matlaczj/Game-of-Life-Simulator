...
int glider(int r, int c, cell_t space[r][c], int k, int l)
{
    ...
    // I know it seems strange but its so that update_print_space wouldnt turn off
    // all the cells before it starts working
    space[k][l].next_state = ON;
    space[k+1][l+1].next_state = ON;
    space[k+2][l].next_state = ON;
    space[k+2][l+1].next_state = ON;
    space[k+1][l+2].next_state = ON;
    return 1;
}
...
int froggy(int r, int c, cell_t space[r][c], int k, int l)
{
    ...
    space[k][l+1].next_state = ON;
    space[k+1][l+1].next_state = ON;
    space[k+2][l+1].next_state = ON;
    space[k+1][l+2].next_state = ON;
    space[k+2][l+2].next_state = ON;
    space[k+3][l+2].next_state = ON;
    return 1;
}
